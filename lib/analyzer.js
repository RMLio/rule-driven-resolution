...
class Analyzer {
  ...
  _findTriplesMapsOfObjectMap(om) {
    let tms = [];
    const poms = this._findPredicateObjectMapsOfObjectMap(om);

    poms.forEach(pom => {
      const temp = this._findTriplesMapsOfPredicateObjectMap(pom);
      tms = tms.concat(temp);
    });
  ...
  _findTriplesMapsOfPredicateMap(pm) {
    const poms = this.mappingStore.getTriples(null, namespaces.rr + 'predicateMap', pm).map(a => a.subject);
    let tms = [];

    poms.forEach(pom => {
      tms = tms.concat(this.mappingStore.getTriples(null, namespaces.rr + 'predicateObjectMap', pom).map(a => a.subject));
    });
