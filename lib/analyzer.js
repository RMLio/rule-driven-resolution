      ...
      if (violation.rdfunitCode) {
        resources.forEach(resource => {
          ...
          if (subjectsOfChange.length > 0) {
            subjectsOfChange.forEach(subjectOfChange => {
              ...
              switch (r.object) {
                ...
                case namespaces.ns1 + 'codeRange':
                  ...
                  break;
                case namespaces.ns1 + 'duplicateLanguageCode':
                  Analyzer._processONELANG(action);
                  break;
                case namespaces.ns1 + 'codeDisjointPredicate':
                  Analyzer._processOWLDISJP(action);
                  break;
                case namespaces.ns1 + 'asymmetricPredicate':
                  Analyzer._processOWL_ASYMP(action);
                  break;
                case namespaces.ns1 + 'irreflexivePredicate':
            ...
            winston.error(`The resource ${resource} does not have an rdf:subject.`);
          }
        });

        violation.possibleActions = Analyzer._filterDuplicatePossibleActions(violation.possibleActions);
