...
class Analyzer {
  ...
  _findPredicateOfPredicateMap(predicateMap) {
    let constants = this.mappingStore.getTriples(predicateMap, namespaces.rr + 'constant', null).map(a => a.object);

    if (constants.length > 0) {
      return constants[0];
    } else {
      let references = this.mappingStore.getTriples(predicateMap, namespaces.rml + 'reference', null).map(a => a.object);

      if (references.length > 0) {
        return references[0];
      } else {
        let templates = this.mappingStore.getTriples(predicateMap, namespaces.rr + 'template', null).map(a => a.object);
        return templates[0];
      }
    }
  }
