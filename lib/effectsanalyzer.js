...
class EffectsAnalyzer extends Analyzer {
  ...
  getEffects(termMap, element, termMapType) {
    let effects = [];
    ...
    switch (termMapType) {
      ...
      default:
        winston.error(`The Term Map with the type ${termMapType} is not supported (element = ${element}.`);
    }
  ...
  _getEffectsOfChangeToTriplesMap(tm, element, type) {
    let effects = [];
