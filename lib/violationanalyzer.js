...
const ontologydefintionsToIgnore = [namespaces.rdfs + 'Literal'];
...
class ViolationAnalyzer extends Analyzer {
  ...
  analyze(addInconsistency = false) {
    ...
    result.forEach(r => {
      ...
      if (violation.rdfunitCode || codes.rml.indexOf(code) !== -1) {
        ...
        violation.involvedOntologyDefinitions = this._findOntologyDefinitions(r.subject);
  ...
  _findOntologyDefinitions(entry) {
    const xs = this.validationStore.getTriples(entry, 'http://www.example.com/x').map(a => a.object);
    const ys = this.validationStore.getTriples(entry, 'http://www.example.com/y').map(a => a.object);
    const zs = this.validationStore.getTriples(entry, 'http://www.example.com/z').map(a => a.object);
    const result = [];

    this._processArguments(xs, result);
    this._processArguments(ys, result);
    this._processArguments(zs, result);

    return result;
  }

  _processArguments(args, result) {
    if (args.length > 0) {
      if (this.mappingStore.getTriples(args[0]).length === 0
        && ontologydefintionsToIgnore.indexOf(args[0]) === -1
        && this.mappingStore.getTriples(null, null, args[0]).length !== 0) {
        result.push(args[0])
      }
    }
  }
