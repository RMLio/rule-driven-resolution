...
class Scorer {
  ...
  getOntologyDefinitionScore(definition) {
    let counter = 0;

    for (let i = 0; i < this.inconsistencies.length; i ++) {
      const inconsistency = this.inconsistencies[i];

      if (inconsistency.ontologyDefinitions.indexOf(definition) !== -1) {
        counter ++;
      }
    }
  ...
  getClusterScore(cluster) {
    ...
    this.inconsistencies.forEach(inconsistency => {
      ...
      while (i < rules.length && inconsistency.rules.indexOf(rules[i]) === -1) {
        i ++;
      }
